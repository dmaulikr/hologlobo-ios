...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == [_mcells count] - 1) {
        ...
        [self willDisplayLastController];
    }
}
...
@synthesize expectedCellWidth = _expectedCellWidth;
...
- (instancetype)init {
    ...
    if(self = [super init]) {
        ...
        _expectedCellWidth = 0.f;
    }
    
    return self;
}

- (CGFloat)expectedCellWidth {
    
    if(_expectedCellWidth == 0.f) {
        
        return self.table.frame.size.width;
    }
    
    return _expectedCellWidth;
}
...
- (void)defaultAction {
    ...
    if(self.hasAction) {
        ...
        NSMethodSignature * msig = [self.target methodSignatureForSelector:self.action];
